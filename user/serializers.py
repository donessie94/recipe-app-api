...
#really useful for complex models such as Django models (database tables) to JSON file that we can send in a http repsonse
...
class UserSerializer(serializers.ModelSerializer):
    ...
    #the serializer nees to know wich model is representing thast why the model is get_user_model()
    class Meta:
        model = get_user_model()

        #these are the list of fields we want to make available trhough the serializer
        #we dont want to include is_staff or is_active because that would imply the user will be able to change those values from its request (when they create objects)
        ...
        #things like do we want the field to be write only? read only? min-length? etc..
                                    ...
                                    #400 bad request is the result of a validation error that we will get here if the password is not 5+ chars
    ...
    #example: the password will be saved as clear text in the default behavior but we want to encript it instead thats why we wont use the default behavior
    def create(self, validated_data):
        """Create and return an user with encrypted password."""
        #we instead will use the create_user after we validated the data above and that will encrypt the passowrd
